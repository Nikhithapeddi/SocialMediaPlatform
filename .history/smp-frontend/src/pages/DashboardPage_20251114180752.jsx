...
const [sidebarOpen, setSidebarOpen] = useState(false);
  ...
  const [viewedUserProfile, setViewedUserProfile] = useState(null); // { user, posts }
...
const parseComments = (comments) => {
  ...
  // Show a user's profile (user id) — fetch user info and their posts
  const showUserProfile = async (id) => {
    try {
      const [userResp, postsResp] = await Promise.all([
        axios.get(`http://localhost:8082/users/by-id/${id}`),
        axios.get(`http://localhost:8082/posts/user/${id}`),
      ]);

      setViewedUserProfile({ user: userResp.data, posts: postsResp.data || [] });
      // Scroll to top of main content so profile is visible
  ...
  const closeViewedProfile = () => setViewedUserProfile(null);
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className="max-w-full mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-6">
            {/* intentionally left blank: email moved to sidebar */}
          </div>
      ...
      <div className="ml-0 md:ml-64 pt-24 pb-12">
        <div className="max-w-3xl mx-auto px-4 md:px-6">
          ...
          {viewedUserProfile && (
            <div className="mb-8 bg-slate-800/50 backdrop-blur border border-indigo-500/20 rounded-xl p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-gradient-to-br from-indigo-400 to-purple-500 rounded-full flex items-center justify-center text-white font-bold text-lg">{(viewedUserProfile.user.username || viewedUserProfile.user.name || 'U').charAt(0).toUpperCase()}</div>
                  <div>
                    <h3 className="text-xl font-bold text-white">{viewedUserProfile.user.username || viewedUserProfile.user.name}</h3>
                    <p className="text-sm text-slate-400">{viewedUserProfile.user.email}</p>
                  </div>
                </div>
                <div className="flex gap-3">
                  {/* Add Post visible only if this is the current logged-in user */}
                  {((viewedUserProfile.user.id || viewedUserProfile.user.userId || '').toString() === (user?.id || user?.userId || localStorage.getItem('userId') || '').toString()) && (
                    <button onClick={() => setShowCreateModal(true)} className="px-4 py-2 rounded-lg bg-indigo-500 text-white font-semibold">Add Post</button>
                  )}
                  <button onClick={closeViewedProfile} className="px-3 py-2 rounded-lg border border-slate-600 text-slate-300">Close</button>
                </div>
              </div>

              {/* Show the user's posts */}
              <div className="space-y-6">
                {viewedUserProfile.posts.length === 0 ? (
                  <p className="text-slate-400">No posts yet.</p>
                ) : (
                  viewedUserProfile.posts.map((post) => (
                    <div key={post.id} className="bg-slate-800/60 rounded-lg overflow-hidden">
  ...
  <div className="px-6 py-4 bg-slate-700/20 border-t border-slate-700/50">
    ...
    <div className="space-y-3 max-h-48 overflow-y-auto">
      ...
      <div className="fixed bottom-4 left-0 right-0 flex justify-center z-50 pointer-events-none">
        <div className="max-w-3xl w-full px-4 md:px-6 pointer-events-auto">
          <div className="bg-slate-900/95 backdrop-blur border border-indigo-500/20 rounded-xl p-3 flex justify-between gap-3 items-center">
            <button onClick={() => setShowCreateModal(true)} className="flex-1 px-4 py-2 rounded-lg bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-semibold">Create Post</button>
            <button onClick={fetchAllUsers} className="flex-1 px-4 py-2 rounded-lg bg-slate-800 text-slate-200 border border-slate-700">Find People</button>
          </div>
        </div>
      </div>
      ...
      {showPeopleModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-800/95 backdrop-blur border border-indigo-500/20 rounded-2xl p-6 max-w-md w-full shadow-2xl">
            <h3 className="text-xl font-bold text-white mb-4">Find People</h3>
            <div className="space-y-3 max-h-64 overflow-y-auto">
              {people.length === 0 ? (
                <p className="text-slate-400">No users found.</p>
              ) : (
                people.map((p) => (
                  <button key={p.id} onClick={async () => { setShowPeopleModal(false); await showUserProfile(p.id); }} className="w-full text-left p-3 rounded-lg hover:bg-indigo-500/5 transition-colors">
                    <p className="text-white font-semibold">{p.username || p.name || p.email}</p>
            ...
            <div className="mt-4 text-right">
              <button onClick={() => setShowPeopleModal(false)} className="px-4 py-2 rounded-lg border border-slate-600 text-slate-300">Close</button>
            </div>
          </div>
        </div>
      )}
