...
export default function FindPeople() {
  ...
  // 👇 NEW FUNCTION — get userId based on email before navigating
  const handleUserClick = async (email) => {
    try {
      const resp = await axios.get(`http://localhost:8082/users/by-email/${email}`);
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {people.map((p) => (
              <button
                key={p.email}
                onClick={() => handleUserClick(p.email)}
